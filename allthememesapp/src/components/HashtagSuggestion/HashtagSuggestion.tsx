...
export const HashtagSuggestion: FunctionComponent<Props> = ({ inputValue, hashtags, currentLine: [currentLine, setCurrentLine], isAllowedNonExistingHashtags = false, coordinates, isStuckedToLeft = false }) => {
  ...
  useEffect(() => {
    if (listRef.current && suggestionRef.current) {
      ...
      const suggestionCoordinates = suggestionRef.current.getBoundingClientRect();

      if (listCoordinates.bottom < suggestionCoordinates.bottom) {
        scrollList(suggestionRef.current.offsetTop - suggestionRef.current.offsetHeight);
      }
  ...
  return (
    <ul
      ...
      style={{
        ...
        width: `min(500px, ${coordinates?.width ? coordinates.width + 26 + 'px' : 'calc(100% - 40px)'})`
      }}
    >
      {!hashtags.length
